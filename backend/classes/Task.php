...
class Task
{
    ...
    public function generate()
    {
        ...
        $this->planning = date("Y-m-d", rand(strtotime("Jan 01 2017"), strtotime("Dec 01 2019")));
        $this->done = rand(0, 1);
        if (rand(0, 1)) {
    ...
    public function load()
    {
        ...
        $props = $dbObj->query("select * from task where id=" . $dbObj->quote($this->id))
    ...
    public function save()
    {
        ...
        $stmt = Config::getDbObj()->prepare("insert into task (created, modified, " . join(',', $flds) . ") values (now(), now(), " . join(',', $qs) . ") on duplicate key update modified=VALUES(modified), " . join(',', $updt));
        $stmt->execute($vals);
        if (! $this->id) {
